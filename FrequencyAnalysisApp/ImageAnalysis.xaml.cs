            ...
            await PrefabsDialog.ViewWaitTask(() => FileSave.Save("csv", async (file) =>
            {
                using (StreamWriter sw = new StreamWriter(await file.OpenStreamForWriteAsync(), FileSave.DefaultEncoding))
                {
                    ...
                    while (isLooping)
                    {
                        ...
                        {
                            await sw.WriteAsync($"{cEnumarator.Current.Value.R};{cEnumarator.Current.Value.G};{cEnumarator.Current.Value.B};{cEnumarator.Current.Value.A};{cEnumarator.Current.Value};{cEnumarator.Current.Count};{cEnumarator.Current.Frequency};");
                            isLooping = true;
                        }
                        else
                            await sw.WriteAsync(";;;;;;;");
